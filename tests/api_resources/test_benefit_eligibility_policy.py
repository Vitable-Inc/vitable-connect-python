...
class TestBenefitEligibilityPolicy:
    ...
    def test_path_params_retrieve(self, client: VitableConnectAPI) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `policy_id` but received ''"):
            client.benefit_eligibility_policy.with_raw_response.retrieve(
                "",
            )


class TestAsyncBenefitEligibilityPolicy:
    ...
    async def test_method_retrieve(self, async_client: AsyncVitableConnectAPI) -> None:
        benefit_eligibility_policy = await async_client.benefit_eligibility_policy.retrieve(
            "epol_abc123def456",
        )
        assert_matches_type(BenefitEligibilityPolicy, benefit_eligibility_policy, path=["response"])
    ...
    async def test_raw_response_retrieve(self, async_client: AsyncVitableConnectAPI) -> None:
        response = await async_client.benefit_eligibility_policy.with_raw_response.retrieve(
            "epol_abc123def456",
        )
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncVitableConnectAPI) -> None:
        async with async_client.benefit_eligibility_policy.with_streaming_response.retrieve(
            "epol_abc123def456",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_retrieve(self, async_client: AsyncVitableConnectAPI) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `policy_id` but received ''"):
            await async_client.benefit_eligibility_policy.with_raw_response.retrieve(
                "",
            )
